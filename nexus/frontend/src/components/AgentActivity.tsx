...
// Events we don't show in the feed
const SILENT = new Set(['main_llm.stream', 'token.update'])
...
function label(event: NexusEvent): string {
  ...
  switch (event.type) {
    ...
    case 'session.done':        return `Done · ${d.nexus_total} nexus vs ${d.naive_estimate} naive (${(d.reduction_pct as number).toFixed(1)}% saved)`
        ...
        {visible.map((event, i) => {
          ...
          return (
            ...
            >
              ...
              <span className={`flex-1 break-all ${s.color} opacity-90`}>
                {label(event)}
              </span>
